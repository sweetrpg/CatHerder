...
public class AccessoryInstance {
    ...
    public static Optional<AccessoryInstance> readInstance(CompoundTag compound) {
        ...
        try {
            ...
            if (KittyTalentsAPI.ACCESSORIES.containsKey(rl)) {
                Accessory type = KittyTalentsAPI.ACCESSORIES.getValue(rl);
                return Optional.of(type.read(compound));
            } else {
                KittyTalentsAPI.LOGGER.warn("Failed to load accessory {}", compound);
            }
        } catch (Exception e) {
            KittyTalentsAPI.LOGGER.warn("Failed to load accessory {}", rl);
        }
    ...
    public ResourceLocation getModelTexture(kittytalents.api.inferface.AbstractCatEntity cat) {
