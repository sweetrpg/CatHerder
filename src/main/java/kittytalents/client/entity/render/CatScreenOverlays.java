...
public class CatScreenOverlays {

    public static final IIngameOverlay FOOD_LEVEL_ELEMENT = (gui, mStack, partialTicks, screenWidth, screenHeight) -> {
        Minecraft mc = Minecraft.getInstance();
        boolean isMounted = mc.player.getVehicle() instanceof kittytalents.common.entity.CatEntity;
        if (isMounted && !mc.options.hideGui && gui.shouldDrawSurvivalElements()) {
            kittytalents.common.entity.CatEntity cat = (kittytalents.common.entity.CatEntity) mc.player.getVehicle();
            ...
            for (int i = 0; i < 10; ++i) {
                ...
                if (cat.hasEffect(MobEffects.HUNGER)) {
                    icon += 36;
                    background = 13;
                }
    ...
    public static final IIngameOverlay AIR_LEVEL_ELEMENT = (gui, mStack, partialTicks, screenWidth, screenHeight) -> {
        ...
        if (isMounted && !mc.options.hideGui && gui.shouldDrawSurvivalElements()) {
            ...
            int maxAir = cat.getMaxAirSupply();
            if (cat.isEyeInFluid(FluidTags.WATER) || air < maxAir) {
