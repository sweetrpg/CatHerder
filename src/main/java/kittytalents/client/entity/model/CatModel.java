...
public class CatModel<T extends kittytalents.api.inferface.AbstractCatEntity> extends ColorableAgeableListModel<T> {
...
//        this.head.texOffs(0, 10).addBox(-0.5F, 0.0F, -5.0F, 3, 3, 4, scaleFactor);
    ...
    public void prepareMobModel(T cat, float limbSwing, float limbSwingAmount, float partialTickTime) {
        this.tail.yRot = cat.getWagAngle(limbSwing, limbSwingAmount, partialTickTime);
...
//                this.legFrontLeft.rotateAngleY = -(float)Math.PI / 10;
        ...
        this.mane.zRot = cat.getShakeAngle(partialTickTime, -0.08F);
