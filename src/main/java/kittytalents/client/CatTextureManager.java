...
public class CatTextureManager extends SimplePreparableReloadListener<kittytalents.client.CatTextureManager.Preparations> {

    public static final kittytalents.client.CatTextureManager INSTANCE = new kittytalents.client.CatTextureManager();
    ...
    public ResourceLocation getTexture(kittytalents.common.entity.CatEntity cat) {
    ...
    public String saveTextureAndLoad(File baseFolder, byte[] data) throws IOException {
        ...
        if (texture == null) {
            ...
            KittyTalents2.LOGGER.debug("Texture not current loaded trying to load");
    ...
    public ResourceLocation getCached(String hash) {
        ...
        if (!this.getRequestStatus(hash).requested()) {
            KittyTalents2.LOGGER.debug("Marked {} cat skin to be requested from the server", hash);
    ...
    private synchronized void loadDogSkinResource(kittytalents.client.CatTextureManager.Preparations prep, Resource resource) {
        ...
        try {
            ...
            if (prep.skinHashToLoc.containsKey(hash)) {
                KittyTalents2.LOGGER.warn("The loaded resource packs contained a duplicate custom cat skin ({} & {})", resource.getLocation(), this.skinHashToLoc.get(hash));
            } else {
                KittyTalents2.LOGGER.info("Found custom cat skin at {} with hash {}", rl, hash);
    ...
    private void loadOverrideData(kittytalents.client.CatTextureManager.Preparations prep, List<Resource> resourcesList) {
        for (Resource iresource : resourcesList) {
            InputStream inputstream = iresource.getInputStream();
            KittyTalents2.LOGGER.debug("Loading {}", iresource);
            try {
    ...
    private synchronized void loadLocaleData(kittytalents.client.CatTextureManager.Preparations prep, InputStream inputStreamIn) {
        ...
        for (Entry<String, JsonElement> entry : jsonobject.entrySet()) {
            ...
            KittyTalents2.LOGGER.info("Loaded override for {} -> {}", hash, texture);
        }
    }

    @Override
    protected kittytalents.client.CatTextureManager.Preparations prepare(ResourceManager resourceManager, ProfilerFiller profiler) {
        kittytalents.client.CatTextureManager.Preparations prep = new kittytalents.client.CatTextureManager.Preparations();
        ...
        for (ResourceLocation rl : resources) {
            try {
                ...
                if (resource == null) {
                    KittyTalents2.LOGGER.warn("Could not get resource");
                    continue;
                }
                ...
                KittyTalents2.LOGGER.warn("Skipped custom cat skin file: {} ({})", rl, exception);
            }
        }
        ...
        try {
            ...
            KittyTalents2.LOGGER.warn("Unable to parse cat skin override data: {}", runtimeexception);
        }
    ...
    @Override
    protected void apply(kittytalents.client.CatTextureManager.Preparations prep, ResourceManager resourceManager, ProfilerFiller profiler) {
        prep.apply(this);
    }
    ...
    protected static class Preparations {
        ...
        public void apply(kittytalents.client.CatTextureManager catTextureManager) {
