...
public class CachedFileTexture extends SimpleTexture {
        ...
        Minecraft.getInstance().execute(() -> {
            if (!this.textureUploaded) {
                ...
                if (this.cacheFile.isFile() && this.cacheFile.exists()) {
                    KittyTalents2.LOGGER.debug("Loading cat texture from local cache ({})", this.cacheFile);
                ...
                if (nativeimage != null) {
                    ...
                    KittyTalents2.LOGGER.warn("Was unable to set image ({})", this.cacheFile);
                }
            }
        });
    }
    ...
    private NativeImage loadTexture(InputStream inputStreamIn) {
       ...
       try {
           ...
           KittyTalents2.LOGGER.warn("Error while loading the skin texture", ioexception);
       }
