package kittytalents.client.event;
...
public class ClientEventHandler {
    ...
    public static void onModelBakeEvent(final ModelBakeEvent event) {
        ...
        try {
            ResourceLocation resourceLocation = ForgeRegistries.BLOCKS.getKey(KittyBlocks.CAT_BED.get());
            ...
            BakedModel customModel = new kittytalents.client.block.model.CatBedModel(event.getModelLoader(), model, model.bake(event.getModelLoader(), model, ForgeModelBakery.defaultTextureGetter(), BlockModelRotation.X180_Y180, unbakedModelLoc, true));

            // Replace all valid block states
            KittyBlocks.CAT_BED.get().getStateDefinition().getPossibleStates().forEach(state -> {
            ...
            KittyTalents2.LOGGER.warn("Could not get base Cat Bed model. Reverting to default textures...");
            e.printStackTrace();
        }
    }
    ...
    public void onInputEvent(final MovementInputUpdateEvent event) {
        if (event.getInput().jumping) {
            ...
            if (event.getPlayer().isPassenger() && entity instanceof kittytalents.common.entity.CatEntity) {
                kittytalents.common.entity.CatEntity cat = (kittytalents.common.entity.CatEntity) entity;
