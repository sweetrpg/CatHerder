package kittytalents.client.screen.widget;
...
public class DogInventoryButton extends Button {
    ...
    public void render(PoseStack stack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.visible) {
            ...
            List<kittytalents.common.entity.CatEntity> dogs = mc.level.getEntitiesOfClass(kittytalents.common.entity.CatEntity.class, mc.player.getBoundingBox().inflate(12D, 12D, 12D),
                (cat) -> cat.canInteract(mc.player) && PackPuppyTalent.hasInventory(cat)
            );
    ...
    public void renderToolTip(PoseStack stack, int mouseX, int mouseY) {
        if (this.active) {
            Component msg = new TranslatableComponent("container.kittytalents.dog_inventories.link");
            this.parent.renderTooltip(stack, msg, mouseX, mouseY);
        } else {
            Component msg = new TranslatableComponent("container.kittytalents.dog_inventories.link").withStyle(ChatFormatting.RED);
            this.parent.renderTooltip(stack, msg, mouseX, mouseY);
        }
    }
}
