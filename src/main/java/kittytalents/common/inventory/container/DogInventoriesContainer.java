...
public class DogInventoriesContainer extends AbstractContainerMenu {
    ...
    public DogInventoriesContainer(int windowId, Inventory playerInventory, SimpleContainerData trackableArray) {
        super(KittyContainerTypes.DOG_INVENTORIES.get(), windowId);
    ...
    public void addDogSlots() {
        ...
        for (int i = 0; i < this.trackableArray.getCount(); i++) {
            ...
            if (entity instanceof kittytalents.common.entity.CatEntity) {
                ...
                PackPuppyItemHandler packInventory = cat.getTalent(KittyTalents.PACK_PUPPY)
                        .map((inst) -> inst.cast(PackPuppyTalent.class).inventory()).orElse(null);
                if (packInventory == null) {
                    continue;
                }

                int level = Mth.clamp(cat.getDogLevel(KittyTalents.PACK_PUPPY), 0, 5); // Number of rows for this cat
                int numCols = Mth.clamp(level, 0, Math.max(0, TOTAL_COLUMNS)); // Number of rows to draw

                for (int row = 0; row < 3; row++) {
                    for (int col = 0; col < numCols; col++) {
                        DogInventorySlot slot = new DogInventorySlot(cat, this.player, packInventory, drawingColumn + col, row, col, col * 3 + row, 8 + 18 * (drawingColumn + col - page), 18 * row + 18);
