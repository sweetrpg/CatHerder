...
public class CatInventoriesContainer extends AbstractContainerMenu {
    ...
    public CatInventoriesContainer(int windowId, Inventory playerInventory, SimpleContainerData trackableArray) {
        super(KittyContainerTypes.CAT_INVENTORIES.get(), windowId);
    ...
    public void addDogSlots() {
        ...
        for (int i = 0; i < this.trackableArray.getCount(); i++) {
            ...
            if (entity instanceof kittytalents.common.entity.CatEntity) {
                ...
                PackKittyItemHandler packInventory = cat.getTalent(KittyTalents.PACK_KITTY)
                        .map((inst) -> inst.cast(PackKittyTalent.class).inventory()).orElse(null);
                if (packInventory == null) {
                    continue;
                }

                int level = Mth.clamp(cat.getCatLevel(KittyTalents.PACK_KITTY), 0, 5); // Number of rows for this cat
