...
public class CatBedRecipe extends CustomRecipe implements IShapedRecipe<CraftingContainer> {
    ...
    public boolean matches(CraftingContainer inv, Level worldIn) {
        ...
        for (int col = 0; col < 3; col++) {
            for (int row = 0; row < 3; row++) {
                if (col == 1 && row < 2) {
                    IBeddingMaterial id = CatBedUtil.getBeddingFromStack(KittyTalentsAPI.BEDDING_MATERIAL, inv.getItem(row * inv.getWidth() + col));
                    ...
                    ICasingMaterial id = CatBedUtil.getCasingFromStack(KittyTalentsAPI.CASING_MATERIAL, inv.getItem(row * inv.getWidth() + col));
    ...
    public ItemStack assemble(CraftingContainer inv) {
        IBeddingMaterial beddingId = CatBedUtil.getBeddingFromStack(KittyTalentsAPI.BEDDING_MATERIAL, inv.getItem(1));
        ICasingMaterial casingId = CatBedUtil.getCasingFromStack(KittyTalentsAPI.CASING_MATERIAL, inv.getItem(0));
