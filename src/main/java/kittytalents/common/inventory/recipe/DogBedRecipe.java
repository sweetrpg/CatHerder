package kittytalents.common.inventory.recipe;
...
public class DogBedRecipe extends CustomRecipe implements IShapedRecipe<CraftingContainer> {
    ...
    public boolean matches(CraftingContainer inv, Level worldIn) {
        ...
        for (int col = 0; col < 3; col++) {
            for (int row = 0; row < 3; row++) {
                if (col == 1 && row < 2) {
                    IBeddingMaterial id = DogBedUtil.getBeddingFromStack(KittyTalentsAPI.BEDDING_MATERIAL, inv.getItem(row * inv.getWidth() + col));
                    ...
                    ICasingMaterial id = DogBedUtil.getCasingFromStack(KittyTalentsAPI.CASING_MATERIAL, inv.getItem(row * inv.getWidth() + col));
    ...
    public ItemStack assemble(CraftingContainer inv) {
        IBeddingMaterial beddingId = DogBedUtil.getBeddingFromStack(KittyTalentsAPI.BEDDING_MATERIAL, inv.getItem(1));
        ICasingMaterial casingId = DogBedUtil.getCasingFromStack(KittyTalentsAPI.CASING_MATERIAL, inv.getItem(0));
    ...
    public RecipeSerializer<?> getSerializer() {
        return KittyRecipeSerializers.CAT_BED.get();
    }
