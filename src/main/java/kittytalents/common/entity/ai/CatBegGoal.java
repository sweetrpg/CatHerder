...
public class CatBegGoal extends Goal {
    ...
    public CatBegGoal(kittytalents.common.entity.CatEntity vanillaCat, float minDistance) {
        this.cat = vanillaCat;
    ...
    private boolean hasTemptationItemInHand(Player player) {
        for (InteractionHand hand : InteractionHand.values()) {
            ...
            if ((this.cat.isTame() ? KittyTags.BEG_ITEMS_TAMED : KittyTags.BEG_ITEMS_UNTAMED).contains(itemstack.getItem())) {
                return true;
            }

            if (KittyTags.TREATS.contains(itemstack.getItem())) {
                return true;
            }
