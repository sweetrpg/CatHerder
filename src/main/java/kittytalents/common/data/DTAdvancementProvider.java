...
public class DTAdvancementProvider extends AdvancementProvider {
    ...
    public String getName() {
        return "KittyTalents Advancements";
    }
    ...
    public void run(HashCache cache) {
        ...
        Advancement advancement = Advancement.Builder.advancement()
                .display(DisplayInfoBuilder.create().icon(KittyItems.TRAINING_TREAT).frame(FrameType.TASK).translate("cat.root").background("stone.png").noToast().noAnnouncement().build())
                .addCriterion("tame_dog", TameAnimalTrigger.TriggerInstance.tamedAnimal(EntityPredicate.Builder.entity().of(KittyEntityTypes.CAT.get()).build()))
                //.withCriterion("get_dog", ItemUseTrigger.TameAnimalTrigger.Instance.create(EntityPredicate.Builder.create().type(KittyEntityTypes.CAT.get()).build()))
        ...
        Advancement advancement2 = Advancement.Builder.advancement()
                .parent(advancement1)
                .display(DisplayInfoBuilder.create().icon(KittyItems.CAPE).frame(FrameType.TASK).translate("cat.accessorise").build())
        ...
        Advancement advancement3 = Advancement.Builder.advancement()
                .parent(advancement2)
                .display(DisplayInfoBuilder.create().icon(KittyItems.RADIO_COLLAR).frame(FrameType.TASK).translate("cat.radio_collar").build())
