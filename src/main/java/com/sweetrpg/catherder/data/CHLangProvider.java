package com.sweetrpg.catherder.data;

import com.sweetrpg.catherder.common.lib.Constants;
import net.minecraft.data.DataGenerator;
import net.minecraftforge.common.data.LanguageProvider;

public class CHLangProvider extends LanguageProvider {
    private final String locale;

    public CHLangProvider(DataGenerator gen, String locale) {
        super(gen, Constants.MOD_ID, locale);
        this.locale = locale;
    }

    @Override
    public String getName() {
        return "CatHerder Language Provider";
    }

    @Override
    protected void addTranslations() {
        switch(this.locale) {
            case Constants.LOCALE_EN_US -> processENUS();
            case Constants.LOCALE_EN_GB -> processENGB();
            case Constants.LOCALE_DE_DE -> processDEDE();
            case Constants.LOCALE_KO_KR -> processKOKR();
            case Constants.LOCALE_RU_RU -> processRURU();
            case Constants.LOCALE_VI_VN -> processVIVN();
            case Constants.LOCALE_ZH_CN -> processZHCN();
            case Constants.LOCALE_ZH_TW -> processZHTW();
        }
    }

    private void processENUS() {
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED, "Cat Bed");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_SET_OWNER_HELP, "Shift click with your cat nearby and they will come and make the bed their own. If the unimaginable happens and they do die right click the bed to revive them.");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_OWNER, "This bed is owned by %s.");
        add(Constants.TRANSLATION_KEY_BLOCK_FOOD_BOWL, "Food Bowl");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BATH, "Cat Bath");
        add(Constants.TRANSLATION_KEY_BLOCK_LITTER_BOX, "Litter Box");
        add(Constants.TRANSLATION_KEY_BLOCK_WILD_CATNIP, "Catnip");
        add(Constants.TRANSLATION_KEY_BLOCK_CARDBOARD_BOX, "Cardboard Box");
        add(Constants.TRANSLATION_KEY_ITEM_THROW_TOY, "Throw Toy");
        add(Constants.TRANSLATION_KEY_ITEM_TRAINING_TREAT, "Training Treat");
        add(Constants.TRANSLATION_KEY_ITEM_SUPER_TREAT, "Super Treat");
        add(Constants.TRANSLATION_KEY_ITEM_MASTER_TREAT, "Master Treat");
        add(Constants.TRANSLATION_KEY_ITEM_DIRE_TREAT, "Dire Treat");
        add(Constants.TRANSLATION_KEY_ITEM_BREEDING_TREAT, "Breeding Treat");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_CHARM, "Cat Charm");
        add(Constants.TRANSLATION_KEY_ITEM_COLLAR_SHEARS, "Collar Shears");
        add(Constants.TRANSLATION_KEY_ITEM_COMMAND_EMBLEM, "Command Emblem");
        add(Constants.TRANSLATION_KEY_ITEM_RADIO_COLLAR, "Radio Collar");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR, "Wool Collar");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR_TOOLTIP, "Color: %s");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_COLLAR, "Creative Collar");
        add(Constants.TRANSLATION_KEY_CREATIVE_COLLAR_TOOLTIP, "Gives Unlimited points to spend");
        add(Constants.TRANSLATION_KEY_ITEM_SPOTTED_COLLAR, "Spotted Collar");
        add(Constants.TRANSLATION_KEY_ITEM_MULTICOLORED_COLLAR, "Multi-colored Collar");
        add(Constants.TRANSLATION_KEY_ITEM_RADAR, "Radar");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_RADAR, "Creative Radar");
        add(Constants.TRANSLATION_KEY_CREATIVE_RADAR_TOOLTIP, "Finds ANY owned cats");
        add(Constants.TRANSLATION_KEY_ITEM_TREAT_BAG, "Treat Bag");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_TOY, "Cat Toy");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE, "Cape");
        add(Constants.TRANSLATION_KEY_ITEM_SUNGLASSES, "Sunglasses");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED, "Colored Cape");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED_TOOLTIP, "Color: %s");
        add(Constants.TRANSLATION_KEY_ITEM_LEATHER_JACKET, "Leather Jacket");
        add(Constants.TRANSLATION_KEY_ITEM_TINY_CATNIP, "Sprig of Catnip");
        add(Constants.TRANSLATION_KEY_ITEM_MEDIUM_CATNIP, "Some Catnip");
        add(Constants.TRANSLATION_KEY_ITEM_BIG_CATNIP, "Bundle of Catnip");
        add(Constants.TRANSLATION_KEY_ITEM_OWNER_CHANGE, "Change Cat Owner");
        add(Constants.TRANSLATION_KEY_OWNER_CHANGE_TOOLTIP, "Sets ANY cat to be yours");
        add(Constants.TRANSLATION_KEY_ITEM_GUARD_SUIT, "Guard Suit");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP_SEEDS, "Catnip Seeds");
        add(Constants.TRANSLATION_KEY_ITEM_WILD_CATNIP, "Catnip");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP, "Catnip");
        add(Constants.TRANSLATION_KEY_ITEM_YARN, "Yarn");
        add(Constants.TRANSLATION_KEY_ITEM_TREATBAG_CONTENTS, "%d %s");
        add(Constants.TRANSLATION_KEY_CONTAINER_PACK_CAT, "Pack Cat");
        add(Constants.TRANSLATION_KEY_CONTAINER_FOOD_BOWL, "Food Bowl");
        add(Constants.TRANSLATION_KEY_CONTAINER_TREAT_BAG, "Treat Bag");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES_LINK, "Cat Inventories");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES, "Cat Inventories");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_MAIN, "Cat Herder");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_CATBED, "Cat Bed Variants");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LOW_LEVEL, "Your cat can't possibly handle the awesome power contained in these treats.");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LEVEL_UP, "Level up!");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_TOO_YOUNG, "This cat's a bit too young to be learning skills just yet.");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_MAX_LEVEL, "CONGRATULATIONS! Your cat has reached the ultimate level!");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_LEVEL_UP, "Level up!");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_TOO_YOUNG, "This cat's a bit too young to be learning skills just yet.");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_MAX_LEVEL, "Your cat is too high level to gain anything from these treats.");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_NAME, "Tomcat");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_DESCRIPTION, "Improves critical hit rate 1/2 a heart for every level plus an extra heart when you master it.");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_NAME, "Catlike Reflexes");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_DESCRIPTION, "Increases the chance that your cat, will completely block a physical attack.");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_NAME, "Bird Catcher");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_DESCRIPTION, "Every level grants a 10%% chance to make anything your cat kills drop its loot twice, Level 5 grants an extra 10%%, bringing it up to a 60%% chance.");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_NAME, "Hell Beast");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_DESCRIPTION, "Allows your cat set things on fire while attacking. The higher the level, the longer the fire lasts. Mastering the skill grants your cat immunity to fire.");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_NAME, "Mount");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_DESCRIPTION, "Allows you to ride your cat anywhere at speed. Every level allows your cat to jump higher. To get on your cat have an empty hand and jump and click on the cat to get on it.");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_NAME, "Pack Cat");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_DESCRIPTION, "Each level gives your cat 3 inventory slots in its, Packcat GUI. To open the GUI right click on your while sneaking! At level 5, your cat will, also automatically pick up any nearby items if, it has space in its inventory.");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_NAME, "Quick Healer");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_DESCRIPTION, "Improves the rate at which your cat heals without needing to rest Mastering the skill will heal your cat 50%% faster when sitting.");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_NAME, "Creeper Sweeper");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_DESCRIPTION, "A cat with this skill can smell creepers in the area and it will growl to warn you. The range at which your cat, can, detect creepers increases by 6 blocks per level, and mastering this skill will make your cat capable of attacking creepers Your cat will reset the creeper's detonation timer with each attack.");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_NAME, "Cheetah Speed");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_DESCRIPTION, "Increases your cat's movement speed while chasing a target and while it is being ridden. Every level grants a 12%% increase, and mastering it grants an additional 15%% increase.");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_NAME, "Fisher Cat");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_DESCRIPTION, "Whenever your cat enters the water, it may catch a fish which it will give to you when it shakes itself dry The chance of catching a fish increases with level, and any points in HellHound may cause the fish to be pre-cooked. Mastering this skill lets your cat breathe underwater.");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_NAME, "Happy Eater");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_DESCRIPTION, "Increases the NP your cat gets from food items by 10%% per level. In addition, level 3 lets your cat eat rotten flesh, and level 5 lets your cat eat fish.");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_NAME, "Bed Finder");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_DESCRIPTION, "While a cat with this skill is on your head, your bed will be highlighted in a black outline. To get the cat on you head, right click on them with a bone in hand. The range at which it can do this is equal to 200 blocks per level, but becomes infinite at level 5.");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_NAME, "Pest Fighter");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_DESCRIPTION, "Inflicts 1 point of damage on all nearby silverfish at random Increasing the skill level increases the range by 3 blocks and mastering the skill doubles the damage.");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_NAME, "Poison Fang");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_DESCRIPTION, "Your cat's attacks will poison their target, and the poison's duration increases with level. At level 3 your cat becomes immune to poison, and mastering this skill allows you to cleanse yourself of all potion effects, including poison, by right-clicking on your cat with a spider eye, at the cost of 30 NP.");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_NAME, "Rescue Cat");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_DESCRIPTION, "If you're at critical health and a cat with this skill is following you, it will attempt to heal you half a heart per level in this skill, at the cost of 100 NP Mastering this skill reduces the cost to 80 NP.");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_NAME, "Razor-sharp Claws");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_COOLDOWN, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_LEVEL, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_MISS, "");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_NAME, "Nermal");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_DESCRIPTION, "This skill will cause your cat to warm the heart of any villager, prompting the villager to give you presents! Higher levels in the skill increase the quality of the random gifts. This can only be done once every two Minecraft days. Mastering the skill allows you to do this once every Minecraft day.");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_NAME, "Pounce");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_EXHAUSTED, "%s: Meooowww, meow, meow mrrrrawwww (Food please!)");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_1, "Villager: Cute cat! Take good care of %s.");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_2, "Here, %s can have some pork.");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_1, "Villager: Awww, who's a good %s?");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_2, "Whoozagood%s? You are! YOU ARE!");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_3, "Yes you are! And a good %s gotta eat!");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_1, "Villager: Oh my goodness! Where did you get that little bundle");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_2, "of happiness and loyalty? Here, take this");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_3, "iron, make some gear, keep your buddy safe!");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_1, "Villager: I think my heart just grew beyond the");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_2, "size of my nose. Here, take this.");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_3, "No, I insist!");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_1, "Villager: I'd just like to tell you that you have");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_2, "the bestest cat in the universe!");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_3, "And also to give you a diamond!");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_1, "Villager: aaaaAAAAAAAAAAAAAAAAAAAAAAAAA");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_2, "AAAAAAAAAAAAAAAAAAAaaaaaaaaaa");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_3, "aaaaaaaaaAAAAAAAAWWWWWWWWW!");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_MOUNT, "Click to make %s dismount");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_VERSION_MIGRATION, "The cat inventory is no longer accessed this way. Open your inventory and click on the cat inventories button.");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING, "Wandering");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_INDICATOR, "(W)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_DESCRIPTION, "In this mode your cat so stay near it's bowl and wait for your next command. To set you cat's bowl bring your cat within a few blocks of it.");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL, "Tactical");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_INDICATOR, "(T)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_DESCRIPTION, "In this mode your cat will follow you but will not attack anything unless you use the command beam");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH, "Skittish");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_INDICATOR, "(S)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK, "Attack");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_INDICATOR, "(A)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_DESCRIPTION, "In this mode your cat will follow you and attack anything that attacks you or that you attack.");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE, "Docile");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_INDICATOR, "(D)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DESCRIPTION, "In this mode your cat will follow you but not attack anything. They are ready for you to throw the toy.");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_NO_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DISTANCE, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL, "Patrol");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_INDICATOR, "(P)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_DESCRIPTION, "In this mode your cat will patrol between 2-4 points (set using a command emblem) and attack monsters that try come into view.");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD, "Guard");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_INDICATOR, "(G)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_DESCRIPTION, "Cats follow closely and attack any monsters that come close.");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE, "Pounce");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_INDICATOR, "(P)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_INDICATOR, "(I)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_HELP, "%s is incapacitated and requires a cake to revive %s.");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE, "Female");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_INDICATOR, "(F)");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_PRONOUN, "her");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_SUBJECT, "she");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_TITLE, "girl");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE, "Male");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_INDICATOR, "(M)");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_PRONOUN, "him");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_SUBJECT, "he");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_TITLE, "boy");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT, "Cat");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNKNOWN_OWNER, "Owner Unknown");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNTAMED, "Untamed");
        add(Constants.TRANSLATION_KEY_COMMAND_COME, "Come!");
        add(Constants.TRANSLATION_KEY_COMMAND_STAY, "Stay!");
        add(Constants.TRANSLATION_KEY_COMMAND_OKAY, "Okay!");
        add(Constants.TRANSLATION_KEY_GUI_PREVIOUS_PAGE, "Previous Page");
        add(Constants.TRANSLATION_KEY_GUI_NEXT_PAGE, "Next Page");
        add(Constants.TRANSLATION_KEY_GUI_NEW_NAME, "New name:");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL, "Level:");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL_DIRE, "Dire level:");
        add(Constants.TRANSLATION_KEY_GUI_POINTS_LEFT, "Points left:");
        add(Constants.TRANSLATION_KEY_GUI_TEXTURE_INDEX, "Texture Index");
        add(Constants.TRANSLATION_KEY_GUI_OBEY_OTHERS, "Obey Others?");
        add(Constants.TRANSLATION_KEY_GUI_FRIENDLY_FIRE, "Friendly Fire?");
        add(Constants.TRANSLATION_KEY_GUI_HEALTH, "Health:");
        add(Constants.TRANSLATION_KEY_GUI_SPEED, "Speed:");
        add(Constants.TRANSLATION_KEY_GUI_OWNER, "Owner:");
        add(Constants.TRANSLATION_KEY_GUI_GENDER, "Gender:");
        add(Constants.TRANSLATION_KEY_GUI_AGE, "Age:");
        add(Constants.TRANSLATION_KEY_GUI_ARMOR, "Armor:");
        add(Constants.TRANSLATION_KEY_GUI_OWNER_YOU, "You");
        add(Constants.TRANSLATION_KEY_GUI_BABY, "Kitten");
        add(Constants.TRANSLATION_KEY_GUI_ADULT, "Adult");
        add(Constants.TRANSLATION_KEY_RADAR_NORTH, "**** %s is %d blocks north ****");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHEAST, "**** %s is %d blocks roughly north east ****");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHWEST, "**** %s is %d blocks roughly north west ****");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTH, "**** %s is %d blocks south ****");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHEAST, "**** %s is %d blocks roughly south east ****");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHWEST, "**** %s is %d blocks roughly south west ****");
        add(Constants.TRANSLATION_KEY_RADAR_EAST, "**** %s is %d blocks east ****");
        add(Constants.TRANSLATION_KEY_RADAR_WEST, "**** %s is %d blocks west ****");
        add(Constants.TRANSLATION_KEY_RADAR_NOT_IN_DIM, "**** Signals coming from %s too ****");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NO_RADIO, "**** Remember to put radio collar on your cat ****");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NULL, "**** No cats found in %s! ****");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_OWN, "**** You do not own %s! Please ensure you have permission to interact with %d! ****");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TITLE, "Cat Herder Configuration");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS, "Cat Behavior Settings");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS_TOOLTIP, "Change certain behaviors of cats.");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL, "General Settings");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL_TOOLTIP, "General settings for the mod.");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS, "Talents");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS_TOOLTIP, "Enable and disable talents");
        add(Constants.TRANSLATION_KEY_ARG_UUID_FORMAT_INVALID, "%s is of the wrong format should contain 4 dashes");
        add(Constants.TRANSLATION_KEY_ARG_UUID_SECTION_INVALID, "%s contains one or more invalid characters");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_SUCCESS, "Summoned %s from the dead");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_FAILURE, "Failed to raise %s from the dead");
        add(Constants.TRANSLATION_KEY_COMMAND_RESPAWN_IMPRECISE, "Too many cats (%d) for this owner under this name, use `byuuid` for %s");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_NULL, "Bedding material not set");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_NULL, "Casing material not set");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MISSING, "Missing: %s");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MISSING, "Missing: %s");
        add(Constants.TRANSLATION_KEY_CATBED_EXPLAIN_MISSING, "This item has likely been spawned in directly!");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_WHITE_WOOL, "White Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_ORANGE_WOOL, "Orange Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_MAGENTA_WOOL, "Magenta Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTBLUE_WOOL, "Light Blue Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_YELLOW_WOOL, "Yellow Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LIME_WOOL, "Lime Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PINK_WOOL, "Pink Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GRAY_WOOL, "Gray Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTGRAY_WOOL, "Light Gray Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_CYAN_WOOL, "Cyan Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PURPLE_WOOL, "Purple Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLUE_WOOL, "Blue Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BROWN_WOOL, "Brown Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GREEN_WOOL, "Green Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_RED_WOOL, "Red Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLACK_WOOL, "Black Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_OAK_PLANKS, "Oak Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_SPRUCE_PLANKS, "Spruce Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_BIRCH_PLANKS, "Birch Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_JUNGLE_PLANKS, "Jungle Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_ACACIA_PLANKS, "Acacia Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_DARKOAK_PLANKS, "Dark Oak Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_CRIMSON_PLANKS, "Crimson Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_WARPED_PLANKS, "Warped Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_AUT_MAPLE_PLANKS, "Maple Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_CHERRY_PLANKS, "Cherry Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_UMBRAN_PLANKS, "Umbran Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_FIR_PLANKS, "Fir Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_PALM_PLANKS, "Palm Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_REDWOOD_PLANKS, "Redwood Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_WILLOW_PLANKS, "Willow Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_HELLBARK_PLANKS, "Hellbark Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_JACARANDA_PLANKS, "Jacaranda Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAHOGANY_PLANKS, "Mahogany Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_EBONY_PLANKS, "Ebony Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAGIC_PLANKS, "Magic Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_DEAD_PLANKS, "Dead Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_SHIMMERWOOD_PLANKS, "Shimmerwood Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_LIVINGWOOD_PLANKS, "Livingwood Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_LIVINGWOOD_PLANKS, "Mossy Livingwood Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_DREAMWOOD_PLANKS, "Dreamwood Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_DREAMWOOD_PLANKS, "Mossy Dreamwood Plank Casing");
    }

    private void processENGB() {
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED, "Cat Bed");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_SET_OWNER_HELP, "Shift click with your cat nearby and they will come and make the bed their own. If the unimaginable happens and they do die right click the bed to revive them.");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_OWNER, "This bed is owned by %s.");
        add(Constants.TRANSLATION_KEY_BLOCK_FOOD_BOWL, "Food Bowl");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BATH, "Cat Bath");
        add(Constants.TRANSLATION_KEY_BLOCK_LITTER_BOX, "Litter Box");
        add(Constants.TRANSLATION_KEY_BLOCK_WILD_CATNIP, "Catnip");
        add(Constants.TRANSLATION_KEY_BLOCK_CARDBOARD_BOX, "Cardboard Box");
        add(Constants.TRANSLATION_KEY_ITEM_THROW_TOY, "Throw Toy");
        add(Constants.TRANSLATION_KEY_ITEM_TRAINING_TREAT, "Training Treat");
        add(Constants.TRANSLATION_KEY_ITEM_SUPER_TREAT, "Super Treat");
        add(Constants.TRANSLATION_KEY_ITEM_MASTER_TREAT, "Master Treat");
        add(Constants.TRANSLATION_KEY_ITEM_DIRE_TREAT, "Dire Treat");
        add(Constants.TRANSLATION_KEY_ITEM_BREEDING_TREAT, "Breeding Treat");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_CHARM, "Cat Charm");
        add(Constants.TRANSLATION_KEY_ITEM_COLLAR_SHEARS, "Collar Shears");
        add(Constants.TRANSLATION_KEY_ITEM_COMMAND_EMBLEM, "Command Emblem");
        add(Constants.TRANSLATION_KEY_ITEM_RADIO_COLLAR, "Radio Collar");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR, "Wool Collar");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR_TOOLTIP, "Color: %s");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_COLLAR, "Creative Collar");
        add(Constants.TRANSLATION_KEY_CREATIVE_COLLAR_TOOLTIP, "Gives Unlimited points to spend");
        add(Constants.TRANSLATION_KEY_ITEM_SPOTTED_COLLAR, "Spotted Collar");
        add(Constants.TRANSLATION_KEY_ITEM_MULTICOLORED_COLLAR, "Multi-coloured Collar");
        add(Constants.TRANSLATION_KEY_ITEM_RADAR, "Radar");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_RADAR, "Creative Radar");
        add(Constants.TRANSLATION_KEY_CREATIVE_RADAR_TOOLTIP, "Finds ANY owned cats");
        add(Constants.TRANSLATION_KEY_ITEM_TREAT_BAG, "Treat Bag");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_TOY, "Cat Toy");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE, "Cape");
        add(Constants.TRANSLATION_KEY_ITEM_SUNGLASSES, "Sunglasses");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED, "Coloured Cape");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED_TOOLTIP, "Colour: %s");
        add(Constants.TRANSLATION_KEY_ITEM_LEATHER_JACKET, "Leather Jacket");
        add(Constants.TRANSLATION_KEY_ITEM_TINY_CATNIP, "Sprig of Catnip");
        add(Constants.TRANSLATION_KEY_ITEM_MEDIUM_CATNIP, "Some Catnip");
        add(Constants.TRANSLATION_KEY_ITEM_BIG_CATNIP, "Bundle of Catnip");
        add(Constants.TRANSLATION_KEY_ITEM_OWNER_CHANGE, "Change Cat Owner");
        add(Constants.TRANSLATION_KEY_OWNER_CHANGE_TOOLTIP, "Sets ANY cat to be yours");
        add(Constants.TRANSLATION_KEY_ITEM_GUARD_SUIT, "Guard Suit");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP_SEEDS, "Catnip Seeds");
        add(Constants.TRANSLATION_KEY_ITEM_WILD_CATNIP, "Catnip");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP, "Catnip");
        add(Constants.TRANSLATION_KEY_ITEM_YARN, "Yarn");
        add(Constants.TRANSLATION_KEY_ITEM_TREATBAG_CONTENTS, "%d %s");
        add(Constants.TRANSLATION_KEY_CONTAINER_PACK_CAT, "Pack Cat");
        add(Constants.TRANSLATION_KEY_CONTAINER_FOOD_BOWL, "Food Bowl");
        add(Constants.TRANSLATION_KEY_CONTAINER_TREAT_BAG, "Treat Bag");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES_LINK, "Cat Inventories");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES, "Cat Inventories");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_MAIN, "Cat Herder");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_CATBED, "Cat Bed Variants");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LOW_LEVEL, "Your cat can't possibly handle the awesome power contained in these treats.");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LEVEL_UP, "Level up!");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_TOO_YOUNG, "This cat's a bit too young to be learning skills just yet.");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_MAX_LEVEL, "CONGRATULATIONS! Your cat has reached the ultimate level!");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_LEVEL_UP, "Level up!");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_TOO_YOUNG, "This cat's a bit too young to be learning skills just yet.");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_MAX_LEVEL, "Your cat is too high level to gain anything from these treats.");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_NAME, "Tomcat");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_DESCRIPTION, "Improves critical hit rate 1/2 a heart for every level plus an extra heart when you master it.");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_NAME, "Catlike Reflexes");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_DESCRIPTION, "Increases the chance that your cat, will completely block a physical attack.");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_NAME, "Bird Catcher");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_DESCRIPTION, "Every level grants a 10%% chance to make anything your cat kills drop its loot twice, Level 5 grants an extra 10%%, bringing it up to a 60%% chance.");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_NAME, "Hell Beast");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_DESCRIPTION, "Allows your cat set things on fire while attacking. The higher the level, the longer the fire lasts. Mastering the skill grants your cat immunity to fire.");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_NAME, "Mount");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_DESCRIPTION, "Allows you to ride your cat anywhere at speed. Every level allows your cat to jump higher. To get on your cat have an empty hand and jump and click on the cat to get on it.");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_NAME, "Pack Cat");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_DESCRIPTION, "Each level gives your cat 3 inventory slots in its, Packcat GUI. To open the GUI right click on your while sneaking! At level 5, your cat will, also automatically pick up any nearby items if, it has space in its inventory.");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_NAME, "Quick Healer");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_DESCRIPTION, "Improves the rate at which your cat heals without needing to rest Mastering the skill will heal your cat 50%% faster when sitting.");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_NAME, "Creeper Sweeper");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_DESCRIPTION, "A cat with this skill can smell creepers in the area and it will growl to warn you. The range at which your cat, can, detect creepers increases by 6 blocks per level, and mastering this skill will make your cat capable of attacking creepers Your cat will reset the creeper's detonation timer with each attack.");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_NAME, "Cheetah Speed");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_DESCRIPTION, "Increases your cat's movement speed while chasing a target and while it is being ridden. Every level grants a 12%% increase, and mastering it grants an additional 15%% increase.");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_NAME, "Fisher Cat");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_DESCRIPTION, "Whenever your cat enters the water, it may catch a fish which it will give to you when it shakes itself dry The chance of catching a fish increases with level, and any points in HellHound may cause the fish to be pre-cooked. Mastering this skill lets your cat breathe underwater.");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_NAME, "Happy Eater");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_DESCRIPTION, "Increases the NP your cat gets from food items by 10%% per level. In addition, level 3 lets your cat eat rotten flesh, and level 5 lets your cat eat fish.");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_NAME, "Bed Finder");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_DESCRIPTION, "While a cat with this skill is on your head, your bed will be highlighted in a black outline. To get the cat on you head, right click on them with a bone in hand. The range at which it can do this is equal to 200 blocks per level, but becomes infinite at level 5.");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_NAME, "Pest Fighter");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_DESCRIPTION, "Inflicts 1 point of damage on all nearby silverfish at random Increasing the skill level increases the range by 3 blocks and mastering the skill doubles the damage.");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_NAME, "Poison Fang");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_DESCRIPTION, "Your cat's attacks will poison their target, and the poison's duration increases with level. At level 3 your cat becomes immune to poison, and mastering this skill allows you to cleanse yourself of all potion effects, including poison, by right-clicking on your cat with a spider eye, at the cost of 30 NP.");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_NAME, "Rescue Cat");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_DESCRIPTION, "If you're at critical health and a cat with this skill is following you, it will attempt to heal you half a heart per level in this skill, at the cost of 100 NP Mastering this skill reduces the cost to 80 NP.");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_NAME, "Razor-sharp Claws");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_COOLDOWN, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_LEVEL, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_MISS, "");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_NAME, "Nermal");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_DESCRIPTION, "This skill will cause your cat to warm the heart of any villager, prompting the villager to give you presents! Higher levels in the skill increase the quality of the random gifts. This can only be done once every two Minecraft days. Mastering the skill allows you to do this once every Minecraft day.");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_NAME, "Pounce");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_EXHAUSTED, "%s: Meooowww, meow, meow mrrrrawwww (Food please!)");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_1, "Villager: Cute cat! Take good care of %s.");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_2, "Here, %s can have some pork.");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_1, "Villager: Awww, who's a good %s?");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_2, "Whoozagood%s? You are! YOU ARE!");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_3, "Yes you are! And a good %s gotta eat!");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_1, "Villager: Oh my goodness! Where did you get that little bundle");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_2, "of happiness and loyalty? Here, take this");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_3, "iron, make some gear, keep your buddy safe!");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_1, "Villager: I think my heart just grew beyond the");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_2, "size of my nose. Here, take this.");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_3, "No, I insist!");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_1, "Villager: I'd just like to tell you that you have");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_2, "the bestest cat in the universe!");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_3, "And also to give you a diamond!");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_1, "Villager: aaaaAAAAAAAAAAAAAAAAAAAAAAAAA");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_2, "AAAAAAAAAAAAAAAAAAAaaaaaaaaaa");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_3, "aaaaaaaaaAAAAAAAAWWWWWWWWW!");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_MOUNT, "Click to make %s dismount");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_VERSION_MIGRATION, "The cat inventory is no longer accessed this way. Open your inventory and click on the cat inventories button.");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING, "Wandering");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_INDICATOR, "(W)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_DESCRIPTION, "In this mode your cat so stay near it's bowl and wait for your next command. To set you cat's bowl bring your cat within a few blocks of it.");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL, "Tactical");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_INDICATOR, "(T)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_DESCRIPTION, "In this mode your cat will follow you but will not attack anything unless you use the command beam");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH, "Skittish");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_INDICATOR, "(S)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK, "Attack");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_INDICATOR, "(A)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_DESCRIPTION, "In this mode your cat will follow you and attack anything that attacks you or that you attack.");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE, "Docile");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_INDICATOR, "(D)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DESCRIPTION, "In this mode your cat will follow you but not attack anything. They are ready for you to throw the toy.");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_NO_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DISTANCE, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL, "Patrol");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_INDICATOR, "(P)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_DESCRIPTION, "In this mode your cat will patrol between 2-4 points (set using a command emblem) and attack monsters that try come into view.");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD, "Guard");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_INDICATOR, "(G)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_DESCRIPTION, "Cats follow closely and attack any monsters that come close.");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE, "Pounce");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_INDICATOR, "(P)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_INDICATOR, "(I)");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_HELP, "%s is incapacitated and requires a cake to revive %s.");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE, "Female");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_INDICATOR, "(F)");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_PRONOUN, "her");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_SUBJECT, "she");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_TITLE, "girl");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE, "Male");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_INDICATOR, "(M)");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_PRONOUN, "him");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_SUBJECT, "he");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_TITLE, "boy");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT, "Cat");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNKNOWN_OWNER, "Owner Unknown");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNTAMED, "Untamed");
        add(Constants.TRANSLATION_KEY_COMMAND_COME, "Come!");
        add(Constants.TRANSLATION_KEY_COMMAND_STAY, "Stay!");
        add(Constants.TRANSLATION_KEY_COMMAND_OKAY, "Okay!");
        add(Constants.TRANSLATION_KEY_GUI_PREVIOUS_PAGE, "Previous Page");
        add(Constants.TRANSLATION_KEY_GUI_NEXT_PAGE, "Next Page");
        add(Constants.TRANSLATION_KEY_GUI_NEW_NAME, "New name:");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL, "Level:");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL_DIRE, "Dire level:");
        add(Constants.TRANSLATION_KEY_GUI_POINTS_LEFT, "Points left:");
        add(Constants.TRANSLATION_KEY_GUI_TEXTURE_INDEX, "Texture Index");
        add(Constants.TRANSLATION_KEY_GUI_OBEY_OTHERS, "Obey Others?");
        add(Constants.TRANSLATION_KEY_GUI_FRIENDLY_FIRE, "Friendly Fire?");
        add(Constants.TRANSLATION_KEY_GUI_HEALTH, "Health:");
        add(Constants.TRANSLATION_KEY_GUI_SPEED, "Speed:");
        add(Constants.TRANSLATION_KEY_GUI_OWNER, "Owner:");
        add(Constants.TRANSLATION_KEY_GUI_GENDER, "Gender:");
        add(Constants.TRANSLATION_KEY_GUI_AGE, "Age:");
        add(Constants.TRANSLATION_KEY_GUI_ARMOR, "Armour:");
        add(Constants.TRANSLATION_KEY_GUI_OWNER_YOU, "You");
        add(Constants.TRANSLATION_KEY_GUI_BABY, "Kitten");
        add(Constants.TRANSLATION_KEY_GUI_ADULT, "Adult");
        add(Constants.TRANSLATION_KEY_RADAR_NORTH, "**** %s is %d blocks north ****");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHEAST, "**** %s is %d blocks roughly north east ****");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHWEST, "**** %s is %d blocks roughly north west ****");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTH, "**** %s is %d blocks south ****");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHEAST, "**** %s is %d blocks roughly south east ****");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHWEST, "**** %s is %d blocks roughly south west ****");
        add(Constants.TRANSLATION_KEY_RADAR_EAST, "**** %s is %d blocks east ****");
        add(Constants.TRANSLATION_KEY_RADAR_WEST, "**** %s is %d blocks west ****");
        add(Constants.TRANSLATION_KEY_RADAR_NOT_IN_DIM, "**** Signals coming from %s too ****");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NO_RADIO, "**** Remember to put radio collar on your cat ****");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NULL, "**** No cats found in %s! ****");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_OWN, "**** You do not own %s! Please ensure you have permission to interact with %d! ****");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TITLE, "Cat Herder Configuration");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS, "Cat Behaviour Settings");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS_TOOLTIP, "Change certain behaviours of cats.");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL, "General Settings");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL_TOOLTIP, "General settings for the mod.");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS, "Talents");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS_TOOLTIP, "Enable and disable talents");
        add(Constants.TRANSLATION_KEY_ARG_UUID_FORMAT_INVALID, "%s is of the wrong format should contain 4 dashes");
        add(Constants.TRANSLATION_KEY_ARG_UUID_SECTION_INVALID, "%s contains one or more invalid characters");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_SUCCESS, "Summoned %s from the dead");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_FAILURE, "Failed to raise %s from the dead");
        add(Constants.TRANSLATION_KEY_COMMAND_RESPAWN_IMPRECISE, "Too many cats (%d) for this owner under this name, use `byuuid` for %s");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_NULL, "Bedding material not set");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_NULL, "Casing material not set");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MISSING, "Missing: %s");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MISSING, "Missing: %s");
        add(Constants.TRANSLATION_KEY_CATBED_EXPLAIN_MISSING, "This item has likely been spawned in directly!");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_WHITE_WOOL, "White Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_ORANGE_WOOL, "Orange Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_MAGENTA_WOOL, "Magenta Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTBLUE_WOOL, "Light Blue Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_YELLOW_WOOL, "Yellow Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LIME_WOOL, "Lime Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PINK_WOOL, "Pink Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GRAY_WOOL, "Gray Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTGRAY_WOOL, "Light Gray Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_CYAN_WOOL, "Cyan Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PURPLE_WOOL, "Purple Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLUE_WOOL, "Blue Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BROWN_WOOL, "Brown Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GREEN_WOOL, "Green Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_RED_WOOL, "Red Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLACK_WOOL, "Black Wool Bedding");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_OAK_PLANKS, "Oak Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_SPRUCE_PLANKS, "Spruce Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_BIRCH_PLANKS, "Birch Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_JUNGLE_PLANKS, "Jungle Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_ACACIA_PLANKS, "Acacia Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_DARKOAK_PLANKS, "Dark Oak Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_CRIMSON_PLANKS, "Crimson Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_WARPED_PLANKS, "Warped Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_AUT_MAPLE_PLANKS, "Maple Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_CHERRY_PLANKS, "Cherry Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_UMBRAN_PLANKS, "Umbran Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_FIR_PLANKS, "Fir Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_PALM_PLANKS, "Palm Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_REDWOOD_PLANKS, "Redwood Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_WILLOW_PLANKS, "Willow Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_HELLBARK_PLANKS, "Hellbark Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_JACARANDA_PLANKS, "Jacaranda Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAHOGANY_PLANKS, "Mahogany Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_EBONY_PLANKS, "Ebony Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAGIC_PLANKS, "Magic Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_DEAD_PLANKS, "Dead Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_SHIMMERWOOD_PLANKS, "Shimmerwood Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_LIVINGWOOD_PLANKS, "Livingwood Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_LIVINGWOOD_PLANKS, "Mossy Livingwood Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_DREAMWOOD_PLANKS, "Dreamwood Plank Casing");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_DREAMWOOD_PLANKS, "Mossy Dreamwood Plank Casing");
    }

    private void processDEDE() {
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_SET_OWNER_HELP, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_FOOD_BOWL, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BATH, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_LITTER_BOX, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_WILD_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CARDBOARD_BOX, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_THROW_TOY, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TRAINING_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SUPER_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MASTER_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_DIRE_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_BREEDING_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_CHARM, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_COLLAR_SHEARS, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_COMMAND_EMBLEM, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_RADIO_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_CREATIVE_COLLAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SPOTTED_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MULTICOLORED_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_RADAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_RADAR, "TODO");
        add(Constants.TRANSLATION_KEY_CREATIVE_RADAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TREAT_BAG, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_TOY, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SUNGLASSES, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_LEATHER_JACKET, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TINY_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MEDIUM_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_BIG_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_OWNER_CHANGE, "TODO");
        add(Constants.TRANSLATION_KEY_OWNER_CHANGE_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_GUARD_SUIT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP_SEEDS, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WILD_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_YARN, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TREATBAG_CONTENTS, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_PACK_CAT, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_FOOD_BOWL, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_TREAT_BAG, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES_LINK, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES, "TODO");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_MAIN, "TODO");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_CATBED, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LOW_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LEVEL_UP, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_TOO_YOUNG, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_MAX_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_LEVEL_UP, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_TOO_YOUNG, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_MAX_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_COOLDOWN, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_LEVEL, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_MISS, "");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_EXHAUSTED, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_MOUNT, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_VERSION_MIGRATION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_NO_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DISTANCE, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_HELP, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_PRONOUN, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_SUBJECT, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_PRONOUN, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_SUBJECT, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNKNOWN_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNTAMED, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_COME, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_STAY, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_OKAY, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_PREVIOUS_PAGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_NEXT_PAGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_NEW_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL_DIRE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_POINTS_LEFT, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_TEXTURE_INDEX, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OBEY_OTHERS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_FRIENDLY_FIRE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_HEALTH, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_SPEED, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_GENDER, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_AGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_ARMOR, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OWNER_YOU, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_BABY, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_ADULT, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTH, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHEAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHWEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTH, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHEAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHWEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_EAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_WEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NOT_IN_DIM, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NO_RADIO, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_OWN, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ARG_UUID_FORMAT_INVALID, "TODO");
        add(Constants.TRANSLATION_KEY_ARG_UUID_SECTION_INVALID, "TODO");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_SUCCESS, "TODO");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_FAILURE, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_RESPAWN_IMPRECISE, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_EXPLAIN_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_WHITE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_ORANGE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_MAGENTA_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTBLUE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_YELLOW_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LIME_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PINK_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GRAY_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTGRAY_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_CYAN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PURPLE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLUE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BROWN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GREEN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_RED_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLACK_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_OAK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_SPRUCE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_BIRCH_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_JUNGLE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_ACACIA_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_DARKOAK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_CRIMSON_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_WARPED_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_AUT_MAPLE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_CHERRY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_UMBRAN_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_FIR_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_PALM_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_REDWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_WILLOW_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_HELLBARK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_JACARANDA_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAHOGANY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_EBONY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAGIC_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_DEAD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_SHIMMERWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_LIVINGWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_LIVINGWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_DREAMWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_DREAMWOOD_PLANKS, "TODO");
    }

    private void processKOKR() {
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_SET_OWNER_HELP, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_FOOD_BOWL, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BATH, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_LITTER_BOX, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_WILD_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CARDBOARD_BOX, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_THROW_TOY, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TRAINING_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SUPER_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MASTER_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_DIRE_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_BREEDING_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_CHARM, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_COLLAR_SHEARS, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_COMMAND_EMBLEM, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_RADIO_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_CREATIVE_COLLAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SPOTTED_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MULTICOLORED_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_RADAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_RADAR, "TODO");
        add(Constants.TRANSLATION_KEY_CREATIVE_RADAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TREAT_BAG, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_TOY, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SUNGLASSES, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_LEATHER_JACKET, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TINY_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MEDIUM_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_BIG_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_OWNER_CHANGE, "TODO");
        add(Constants.TRANSLATION_KEY_OWNER_CHANGE_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_GUARD_SUIT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP_SEEDS, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WILD_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_YARN, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TREATBAG_CONTENTS, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_PACK_CAT, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_FOOD_BOWL, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_TREAT_BAG, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES_LINK, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES, "TODO");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_MAIN, "TODO");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_CATBED, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LOW_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LEVEL_UP, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_TOO_YOUNG, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_MAX_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_LEVEL_UP, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_TOO_YOUNG, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_MAX_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_COOLDOWN, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_LEVEL, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_MISS, "");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_EXHAUSTED, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_MOUNT, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_VERSION_MIGRATION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_NO_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DISTANCE, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_HELP, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_PRONOUN, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_SUBJECT, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_PRONOUN, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_SUBJECT, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNKNOWN_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNTAMED, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_COME, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_STAY, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_OKAY, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_PREVIOUS_PAGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_NEXT_PAGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_NEW_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL_DIRE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_POINTS_LEFT, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_TEXTURE_INDEX, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OBEY_OTHERS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_FRIENDLY_FIRE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_HEALTH, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_SPEED, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_GENDER, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_AGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_ARMOR, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OWNER_YOU, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_BABY, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_ADULT, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTH, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHEAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHWEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTH, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHEAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHWEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_EAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_WEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NOT_IN_DIM, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NO_RADIO, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_OWN, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ARG_UUID_FORMAT_INVALID, "TODO");
        add(Constants.TRANSLATION_KEY_ARG_UUID_SECTION_INVALID, "TODO");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_SUCCESS, "TODO");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_FAILURE, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_RESPAWN_IMPRECISE, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_EXPLAIN_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_WHITE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_ORANGE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_MAGENTA_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTBLUE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_YELLOW_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LIME_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PINK_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GRAY_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTGRAY_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_CYAN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PURPLE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLUE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BROWN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GREEN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_RED_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLACK_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_OAK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_SPRUCE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_BIRCH_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_JUNGLE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_ACACIA_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_DARKOAK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_CRIMSON_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_WARPED_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_AUT_MAPLE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_CHERRY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_UMBRAN_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_FIR_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_PALM_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_REDWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_WILLOW_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_HELLBARK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_JACARANDA_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAHOGANY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_EBONY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAGIC_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_DEAD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_SHIMMERWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_LIVINGWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_LIVINGWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_DREAMWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_DREAMWOOD_PLANKS, "TODO");
    }

    private void processRURU() {
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_SET_OWNER_HELP, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_FOOD_BOWL, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BATH, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_LITTER_BOX, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_WILD_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CARDBOARD_BOX, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_THROW_TOY, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TRAINING_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SUPER_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MASTER_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_DIRE_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_BREEDING_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_CHARM, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_COLLAR_SHEARS, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_COMMAND_EMBLEM, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_RADIO_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_CREATIVE_COLLAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SPOTTED_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MULTICOLORED_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_RADAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_RADAR, "TODO");
        add(Constants.TRANSLATION_KEY_CREATIVE_RADAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TREAT_BAG, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_TOY, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SUNGLASSES, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_LEATHER_JACKET, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TINY_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MEDIUM_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_BIG_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_OWNER_CHANGE, "TODO");
        add(Constants.TRANSLATION_KEY_OWNER_CHANGE_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_GUARD_SUIT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP_SEEDS, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WILD_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_YARN, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TREATBAG_CONTENTS, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_PACK_CAT, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_FOOD_BOWL, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_TREAT_BAG, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES_LINK, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES, "TODO");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_MAIN, "TODO");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_CATBED, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LOW_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LEVEL_UP, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_TOO_YOUNG, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_MAX_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_LEVEL_UP, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_TOO_YOUNG, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_MAX_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_COOLDOWN, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_LEVEL, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_MISS, "");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_EXHAUSTED, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_MOUNT, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_VERSION_MIGRATION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_NO_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DISTANCE, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_HELP, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_PRONOUN, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_SUBJECT, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_PRONOUN, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_SUBJECT, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNKNOWN_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNTAMED, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_COME, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_STAY, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_OKAY, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_PREVIOUS_PAGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_NEXT_PAGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_NEW_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL_DIRE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_POINTS_LEFT, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_TEXTURE_INDEX, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OBEY_OTHERS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_FRIENDLY_FIRE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_HEALTH, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_SPEED, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_GENDER, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_AGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_ARMOR, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OWNER_YOU, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_BABY, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_ADULT, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTH, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHEAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHWEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTH, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHEAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHWEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_EAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_WEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NOT_IN_DIM, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NO_RADIO, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_OWN, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ARG_UUID_FORMAT_INVALID, "TODO");
        add(Constants.TRANSLATION_KEY_ARG_UUID_SECTION_INVALID, "TODO");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_SUCCESS, "TODO");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_FAILURE, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_RESPAWN_IMPRECISE, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_EXPLAIN_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_WHITE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_ORANGE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_MAGENTA_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTBLUE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_YELLOW_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LIME_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PINK_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GRAY_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTGRAY_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_CYAN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PURPLE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLUE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BROWN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GREEN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_RED_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLACK_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_OAK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_SPRUCE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_BIRCH_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_JUNGLE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_ACACIA_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_DARKOAK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_CRIMSON_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_WARPED_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_AUT_MAPLE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_CHERRY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_UMBRAN_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_FIR_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_PALM_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_REDWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_WILLOW_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_HELLBARK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_JACARANDA_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAHOGANY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_EBONY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAGIC_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_DEAD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_SHIMMERWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_LIVINGWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_LIVINGWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_DREAMWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_DREAMWOOD_PLANKS, "TODO");
    }

    private void processVIVN() {
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_SET_OWNER_HELP, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_FOOD_BOWL, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BATH, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_LITTER_BOX, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_WILD_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CARDBOARD_BOX, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_THROW_TOY, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TRAINING_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SUPER_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MASTER_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_DIRE_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_BREEDING_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_CHARM, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_COLLAR_SHEARS, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_COMMAND_EMBLEM, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_RADIO_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_CREATIVE_COLLAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SPOTTED_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MULTICOLORED_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_RADAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_RADAR, "TODO");
        add(Constants.TRANSLATION_KEY_CREATIVE_RADAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TREAT_BAG, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_TOY, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SUNGLASSES, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_LEATHER_JACKET, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TINY_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MEDIUM_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_BIG_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_OWNER_CHANGE, "TODO");
        add(Constants.TRANSLATION_KEY_OWNER_CHANGE_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_GUARD_SUIT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP_SEEDS, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WILD_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_YARN, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TREATBAG_CONTENTS, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_PACK_CAT, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_FOOD_BOWL, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_TREAT_BAG, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES_LINK, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES, "TODO");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_MAIN, "TODO");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_CATBED, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LOW_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LEVEL_UP, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_TOO_YOUNG, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_MAX_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_LEVEL_UP, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_TOO_YOUNG, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_MAX_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_COOLDOWN, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_LEVEL, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_MISS, "");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_EXHAUSTED, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_MOUNT, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_VERSION_MIGRATION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_NO_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DISTANCE, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_HELP, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_PRONOUN, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_SUBJECT, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_PRONOUN, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_SUBJECT, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNKNOWN_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNTAMED, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_COME, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_STAY, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_OKAY, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_PREVIOUS_PAGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_NEXT_PAGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_NEW_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL_DIRE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_POINTS_LEFT, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_TEXTURE_INDEX, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OBEY_OTHERS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_FRIENDLY_FIRE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_HEALTH, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_SPEED, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_GENDER, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_AGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_ARMOR, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OWNER_YOU, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_BABY, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_ADULT, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTH, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHEAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHWEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTH, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHEAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHWEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_EAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_WEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NOT_IN_DIM, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NO_RADIO, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_OWN, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ARG_UUID_FORMAT_INVALID, "TODO");
        add(Constants.TRANSLATION_KEY_ARG_UUID_SECTION_INVALID, "TODO");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_SUCCESS, "TODO");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_FAILURE, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_RESPAWN_IMPRECISE, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_EXPLAIN_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_WHITE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_ORANGE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_MAGENTA_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTBLUE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_YELLOW_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LIME_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PINK_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GRAY_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTGRAY_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_CYAN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PURPLE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLUE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BROWN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GREEN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_RED_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLACK_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_OAK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_SPRUCE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_BIRCH_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_JUNGLE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_ACACIA_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_DARKOAK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_CRIMSON_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_WARPED_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_AUT_MAPLE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_CHERRY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_UMBRAN_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_FIR_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_PALM_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_REDWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_WILLOW_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_HELLBARK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_JACARANDA_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAHOGANY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_EBONY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAGIC_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_DEAD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_SHIMMERWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_LIVINGWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_LIVINGWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_DREAMWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_DREAMWOOD_PLANKS, "TODO");
    }

    private void processZHCN() {
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_SET_OWNER_HELP, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_FOOD_BOWL, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BATH, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_LITTER_BOX, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_WILD_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CARDBOARD_BOX, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_THROW_TOY, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TRAINING_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SUPER_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MASTER_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_DIRE_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_BREEDING_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_CHARM, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_COLLAR_SHEARS, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_COMMAND_EMBLEM, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_RADIO_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_CREATIVE_COLLAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SPOTTED_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MULTICOLORED_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_RADAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_RADAR, "TODO");
        add(Constants.TRANSLATION_KEY_CREATIVE_RADAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TREAT_BAG, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_TOY, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SUNGLASSES, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_LEATHER_JACKET, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TINY_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MEDIUM_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_BIG_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_OWNER_CHANGE, "TODO");
        add(Constants.TRANSLATION_KEY_OWNER_CHANGE_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_GUARD_SUIT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP_SEEDS, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WILD_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_YARN, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TREATBAG_CONTENTS, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_PACK_CAT, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_FOOD_BOWL, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_TREAT_BAG, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES_LINK, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES, "TODO");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_MAIN, "TODO");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_CATBED, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LOW_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LEVEL_UP, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_TOO_YOUNG, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_MAX_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_LEVEL_UP, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_TOO_YOUNG, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_MAX_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_COOLDOWN, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_LEVEL, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_MISS, "");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_EXHAUSTED, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_MOUNT, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_VERSION_MIGRATION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_NO_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DISTANCE, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_HELP, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_PRONOUN, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_SUBJECT, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_PRONOUN, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_SUBJECT, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNKNOWN_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNTAMED, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_COME, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_STAY, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_OKAY, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_PREVIOUS_PAGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_NEXT_PAGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_NEW_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL_DIRE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_POINTS_LEFT, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_TEXTURE_INDEX, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OBEY_OTHERS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_FRIENDLY_FIRE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_HEALTH, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_SPEED, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_GENDER, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_AGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_ARMOR, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OWNER_YOU, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_BABY, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_ADULT, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTH, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHEAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHWEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTH, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHEAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHWEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_EAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_WEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NOT_IN_DIM, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NO_RADIO, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_OWN, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ARG_UUID_FORMAT_INVALID, "TODO");
        add(Constants.TRANSLATION_KEY_ARG_UUID_SECTION_INVALID, "TODO");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_SUCCESS, "TODO");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_FAILURE, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_RESPAWN_IMPRECISE, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_EXPLAIN_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_WHITE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_ORANGE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_MAGENTA_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTBLUE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_YELLOW_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LIME_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PINK_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GRAY_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTGRAY_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_CYAN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PURPLE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLUE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BROWN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GREEN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_RED_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLACK_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_OAK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_SPRUCE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_BIRCH_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_JUNGLE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_ACACIA_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_DARKOAK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_CRIMSON_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_WARPED_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_AUT_MAPLE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_CHERRY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_UMBRAN_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_FIR_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_PALM_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_REDWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_WILLOW_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_HELLBARK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_JACARANDA_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAHOGANY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_EBONY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAGIC_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_DEAD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_SHIMMERWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_LIVINGWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_LIVINGWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_DREAMWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_DREAMWOOD_PLANKS, "TODO");
    }

    private void processZHTW() {
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_SET_OWNER_HELP, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BED_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_FOOD_BOWL, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CAT_BATH, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_LITTER_BOX, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_WILD_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_BLOCK_CARDBOARD_BOX, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_THROW_TOY, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TRAINING_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SUPER_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MASTER_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_DIRE_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_BREEDING_TREAT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_CHARM, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_COLLAR_SHEARS, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_COMMAND_EMBLEM, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_RADIO_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WOOL_COLLAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_CREATIVE_COLLAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SPOTTED_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MULTICOLORED_COLLAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_RADAR, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CREATIVE_RADAR, "TODO");
        add(Constants.TRANSLATION_KEY_CREATIVE_RADAR_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TREAT_BAG, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAT_TOY, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_SUNGLASSES, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CAPE_COLORED_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_LEATHER_JACKET, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TINY_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_MEDIUM_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_BIG_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_OWNER_CHANGE, "TODO");
        add(Constants.TRANSLATION_KEY_OWNER_CHANGE_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_GUARD_SUIT, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP_SEEDS, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_WILD_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_CATNIP, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_YARN, "TODO");
        add(Constants.TRANSLATION_KEY_ITEM_TREATBAG_CONTENTS, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_PACK_CAT, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_FOOD_BOWL, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_TREAT_BAG, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES_LINK, "TODO");
        add(Constants.TRANSLATION_KEY_CONTAINER_INVENTORIES, "TODO");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_MAIN, "TODO");
        add(Constants.TRANSLATION_KEY_ITEMGROUP_CATBED, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LOW_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_LEVEL_UP, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_TOO_YOUNG, "TODO");
        add(Constants.TRANSLATION_KEY_DIRE_TREAT_MAX_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_LEVEL_UP, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_TOO_YOUNG, "TODO");
        add(Constants.TRANSLATION_KEY_NORMAL_TREAT_MAX_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_TOMCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CATLIKEREFLEXES_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BIRDCATCHER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HELLBEAST_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_QUICKHEALER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CREEPERSWEEPER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_CHEETAHSPEED_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_FISHERCAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_HAPPYEATER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PESTFIGHTER_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POISONFANG_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RESCUECAT_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_COOLDOWN, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_LEVEL, "");
        add(Constants.TRANSLATION_KEY_TALENT_RAZORSHARPCLAWS_MISS, "");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_TALENT_MOUNT_EXHAUSTED, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_1_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_2_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_3_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_4_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_5_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_1, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_2, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_NERMAL_MSG_6_LINE_3, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_BEDFINDER_MOUNT, "TODO");
        add(Constants.TRANSLATION_KEY_TALENT_PACKCAT_VERSION_MIGRATION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_WANDERING_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_TACTICAL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_SKITTISH_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_ATTACK_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_NO_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_BOWL, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_DOCILE_DISTANCE, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_PATROL_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_GUARD_DESCRIPTION, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_POUNCE_DESCRIPTION, "");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_MODE_INCAPACITATED_HELP, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_PRONOUN, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_SUBJECT, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_FEMALE_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_INDICATOR, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_PRONOUN, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_SUBJECT, "TODO");
        add(Constants.TRANSLATION_KEY_CAT_GENDER_MALE_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNKNOWN_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_ENTITY_CAT_UNTAMED, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_COME, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_STAY, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_OKAY, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_PREVIOUS_PAGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_NEXT_PAGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_NEW_NAME, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_LEVEL_DIRE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_POINTS_LEFT, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_TEXTURE_INDEX, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OBEY_OTHERS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_FRIENDLY_FIRE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_HEALTH, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_SPEED, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OWNER, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_GENDER, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_AGE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_ARMOR, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_OWNER_YOU, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_BABY, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_ADULT, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTH, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHEAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NORTHWEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTH, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHEAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_SOUTHWEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_EAST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_WEST, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_NOT_IN_DIM, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NO_RADIO, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_RADAR_ERROR_OWN, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TITLE, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_CAT_SETTINGS_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_GENERAL_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS, "TODO");
        add(Constants.TRANSLATION_KEY_GUI_CONFIG_TALENTS_TOOLTIP, "TODO");
        add(Constants.TRANSLATION_KEY_ARG_UUID_FORMAT_INVALID, "TODO");
        add(Constants.TRANSLATION_KEY_ARG_UUID_SECTION_INVALID, "TODO");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_SUCCESS, "TODO");
        add(Constants.TRANSLATION_KEY_COMMANDS_RESPAWN_UUID_FAILURE, "TODO");
        add(Constants.TRANSLATION_KEY_COMMAND_RESPAWN_IMPRECISE, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_NULL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_EXPLAIN_MISSING, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_WHITE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_ORANGE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_MAGENTA_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTBLUE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_YELLOW_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LIME_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PINK_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GRAY_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_LTGRAY_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_CYAN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_PURPLE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLUE_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BROWN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_GREEN_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_RED_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_BEDDING_MC_BLACK_WOOL, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_OAK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_SPRUCE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_BIRCH_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_JUNGLE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_ACACIA_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_DARKOAK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_CRIMSON_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_MC_WARPED_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_AUT_MAPLE_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_CHERRY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_UMBRAN_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_FIR_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_PALM_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_REDWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_WILLOW_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_HELLBARK_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_JACARANDA_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAHOGANY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_EBONY_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_MAGIC_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOP_DEAD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_SHIMMERWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_LIVINGWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_LIVINGWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_DREAMWOOD_PLANKS, "TODO");
        add(Constants.TRANSLATION_KEY_CATBED_CASING_BOTANIA_MOSSY_DREAMWOOD_PLANKS, "TODO");
    }

}
