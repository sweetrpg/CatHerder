...
public class DTAdvancementProvider extends AdvancementProvider {
    ...
    public void run(HashCache cache) {
        ...
        Advancement advancement = Advancement.Builder.advancement()
                .display(DisplayInfoBuilder.create().icon(CatItems.TRAINING_TREAT).frame(FrameType.TASK).translate("cat.root").background("stone.png").noToast().noAnnouncement().build())
                .addCriterion("tame_dog", TameAnimalTrigger.TriggerInstance.tamedAnimal(EntityPredicate.Builder.entity().of(CatEntityTypes.CAT.get()).build()))
        ...
        Advancement advancement2 = Advancement.Builder.advancement()
                .parent(advancement1)
                .display(DisplayInfoBuilder.create().icon(CatItems.CAPE).frame(FrameType.TASK).translate("cat.accessorise").build())
        ...
        Advancement advancement3 = Advancement.Builder.advancement()
                .parent(advancement2)
                .display(DisplayInfoBuilder.create().icon(CatItems.RADIO_COLLAR).frame(FrameType.TASK).translate("cat.radio_collar").build())
