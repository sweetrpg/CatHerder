...
public class WildCropGeneration
{
	...
	public static void registerWildCropGeneration() {
		FEATURE_PATCH_WILD_CATNIP = Registry.register(BuiltinRegistries.CONFIGURED_FEATURE, new ResourceLocation(Constants.MOD_ID, "patch_wild_catnip"),
				Feature.RANDOM_PATCH.configured(getWildCropConfiguration(ModBlocks.WILD_CATNIP.get(), 64, 4, BlockPredicate.matchesBlock(Blocks.GRASS_BLOCK, BLOCK_BELOW))));

		PATCH_WILD_CATNIP = Registry.register(BuiltinRegistries.PLACED_FEATURE, new ResourceLocation(Constants.MOD_ID, "patch_wild_catnip"),
				FEATURE_PATCH_WILD_CATNIP.placed(RarityFilter.onAverageOnceEvery(ConfigHandler.SERVER.CHANCE_WILD_CATNIP.get()), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP, BiomeFilter.biome()));
	}
}
