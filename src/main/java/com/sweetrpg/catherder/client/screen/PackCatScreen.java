...
public class PackCatScreen extends AbstractContainerScreen<PackCatContainer> {
    ...
    protected void renderBg(GuiGraphics graphics, float partialTicks, int xMouse, int yMouse) {
        ...
        graphics.blit(Resources.GUI_PACK_CAT, l, i1, 0, 0, this.imageWidth, this.imageHeight);

        for (int j1 = 0; j1 < 3; j1++)
            for (int k1 = 0; k1 < Mth.clamp(this.getMenu().getCatLevel(), 0, 5); k1++)
                graphics.blit(Resources.GUI_PACK_CAT, l + 78 + 18 * k1, i1 + 9 + 18 * j1 + 15, 197, 2, 18, 18);

        InventoryScreen.renderEntityInInventoryFollowsMouse(graphics, l + 42, i1 + 51, 30, (float)(l + 51) - xMouse, (float)((i1 + 75) - 50) - yMouse, this.getMenu().getCat());
    }
}
