...
public class ArmorAccessoryRenderer extends RenderLayer<CatEntity, CatModel<CatEntity>> {
    ...
    public static <T extends LivingEntity> void renderArmorCutout(EntityModel<T> modelIn, ResourceLocation textureLocationIn, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, T entityIn, float red, float green, float blue, boolean enchanted) {
        VertexConsumer vertexBuilder = ItemRenderer.getArmorFoilBuffer(bufferIn, RenderType.armorCutoutNoCull(textureLocationIn), false, enchanted);
        modelIn.renderToBuffer(matrixStackIn, vertexBuilder, packedLightIn, OverlayTexture.NO_OVERLAY, red, green, blue, 1.0F);
    }
        ...
        for(AccessoryInstance accessoryInst : cat.getAccessories()) {
            if(accessoryInst instanceof ArmourAccessory.Instance armorInst) {
                ...
                if(accessoryInst instanceof IColoredObject) {
                    ...
                    renderArmorCutout(this.model, armorInst.getModelTexture(cat), poseStack, buffer, packedLight, cat, color[0], color[1], color[2], armorInst.hasEffect());
                }
                else {
                    renderArmorCutout(this.model, armorInst.getModelTexture(cat), poseStack, buffer, packedLight, cat, 1.0F, 1.0F, 1.0F, armorInst.hasEffect());
                }
            }
        }
    }
